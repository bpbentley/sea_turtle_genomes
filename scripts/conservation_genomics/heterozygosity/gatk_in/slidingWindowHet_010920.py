...
cc=open(sys.argv[6], 'r')
chrom_size={line.strip().split('\t')[0]:line.strip().split('\t')[1] for line in cc}
cc.close()


# Get list of samples from VCF file header (need line = line.decode() or line.decode('utf-8')
samples=[]
for line in VCF:
...
output.write('chrom\twindow_start\tsites_total\tcalls\thets\n')
# % ('\tcalls', '\thets') )
