#!/bin/bash
#BSUB -J "CheMyd_GATK_exons[1-28]"
#BSUB -e ./logs/GATK/exons/CheMyd_GATK_exons.%I.%J.err
#BSUB -o ./logs/GATK/exons/CheMyd_GATK_exons.%I.%J.log
#BSUB -W 90:00
#BSUB -n 20
...
NAME=$(sed -n 1p ./WGR_greens.txt | cut -f1) # unique name needed if running multiple pipelines on different data, as files will be written to same directory, and will over-write if not unique.
...
OUTDIR=/project/uma_lisa_komoroske/Blair/WGR_for_genome/GATK_out/CheMyd/${NAME}/exons
TEMPDIR=/project/uma_lisa_komoroske/Blair/WGR_for_genome/GATK_out/CheMyd/${NAME}/exons/temp
...
BAM=${NAME}_SORT_DR_RG_IR.bam # samtools index bam file first!
...
MINDEPTH=$(sed -n 1p ./WGR_greens.txt | cut -f3)
MAXDEPTH=$(sed -n 1p ./WGR_greens.txt | cut -f4)
