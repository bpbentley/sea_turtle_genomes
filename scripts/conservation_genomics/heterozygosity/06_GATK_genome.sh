#!/bin/bash
#BSUB -J "CheMyd_GATK_genome[1-28]"
#BSUB -e ./logs/GATK/CheMyd_GATK_genome.%I.%J.err
#BSUB -o ./logs/GATK/CheMyd_GATK_genome.%I.%J.log
#BSUB -W 90:00
#BSUB -n 16
...
NAME=$(sed -n 4p ./WGR_greens.txt | cut -f1) # unique name needed if running multiple pipelines on different data, as files will be written to same directory, and will over-write if not unique.
...
OUTDIR=/project/uma_lisa_komoroske/Blair/WGR_for_genome/GATK_out/CheMyd/${NAME}
TEMPDIR=/project/uma_lisa_komoroske/Blair/WGR_for_genome/GATK_out/CheMyd/${NAME}/temp
...
MINDEPTH=$(sed -n 4p ./WGR_greens.txt | cut -f3)
MAXDEPTH=$(sed -n 4p ./WGR_greens.txt | cut -f4)
