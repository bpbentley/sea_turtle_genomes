#!/bin/bash
#BSUB -J "rDerCor_GATK_masked[1-28]"
#BSUB -e ./logs/07_GATK_masked/rDerCor_GATK_masked.%I.err
#BSUB -o ./logs/07_GATK_masked/rDerCor_GATK_masked.%I.log
#BSUB -W 90:00
#BSUB -n 20
...
NAME=rDerCor1_masked # unique name needed if running multiple pipelines on different data, as files will be written to same directory, and will over-write if not unique.

# Directories
SCRIPTDIR=/project/uma_lisa_komoroske/Blair/rDerCor1/scripts/gatk_in
BAMDIR=/project/uma_lisa_komoroske/Blair/rDerCor1/bams/BWA_rDerCor1_20210524
OUTDIR=/project/uma_lisa_komoroske/Blair/rDerCor1/GATK_out/masked
TEMPDIR=/project/uma_lisa_komoroske/Blair/rDerCor1/GATK_out/masked/temp
...
SCAFFOLDLIST=rDerCor1.pri.cur.20210524.scaffold.list.txt # list must have ">" removed from before each scaffold! SEE README FILE FOR HOW TO GENERATE THE SCAFFOLDLIST FROM THE FASTA FILE
CHRLENGTHS=${REFDIR}/rDerCor1.pri.cur.20210524.scaffold.lengths.txt # SEE README FILE FOR HOW TO GENERATE THE CHROMOSOME LENGTHS FILE FROM THE FASTA INDEX FILE
BAM=rDerCor1_20210524_SORT_DR_RG.bam # samtools index bam file first!
